...
export function gridLayout(pages:readonly PageSize[],cols:number,viewportWidth:number,zoom:number,notes?:{gutter:number;bottom:(index:number,scale:number)=>number}){
  ...
  const gap=24,pad=28,gutter=notes?.gutter||0;const fit=Math.max(.04,(viewportWidth-pad*2-cols*gutter)/(cols*width+(cols-1)*gap));const scale=fit*zoom;
  const gridWidth=(cols*width+(cols-1)*gap)*scale+cols*gutter;
  ...
  for(let start=0;start<pages.length;start+=cols){
    ...
    row.forEach((height,col)=>items.push({left:left+col*((width+gap)*scale+gutter),top,width:width*scale,height:height*scale}));
    top+=Math.max(...row.map((h,col)=>Math.max(h*scale,notes?.bottom(start+col,width/pages[start+col].width*scale)||0)))+gap*scale;
  }
