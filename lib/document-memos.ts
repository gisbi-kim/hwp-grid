export type DocumentMemo={id:string;author:string;text:string;section:number;paragraph:number;inTable:boolean;page?:number};
...
export function readSectionMemos(xml:string,section:number):DocumentMemo[]{
 ...
 paragraphs.forEach((p,paragraph)=>{
  for(const field of Array.from(p.getElementsByTagNameNS('*','fieldBegin'))){
   ...
   result.push({id:`${section}-${field.getAttribute('id')||result.length}`,author,text,section,paragraph,inTable});
  }
 });return result;
}
