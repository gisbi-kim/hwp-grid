...
export function RuntimeStats({documentMs}:{documentMs:number|null}){
  ...
  return <div className="runtime-stats" role="group" aria-label="로딩 시간 및 메모리 정보">
    ...
    <details><summary>측정 안내</summary><p>최초 로딩은 이번 접속 시작부터 사이트 로드 완료까지입니다. 문서 준비는 파일 읽기·글꼴·엔진 초기화·문서 분석 시간이며, 페이지 그림 표시와 저장 시간은 제외합니다. 메모리는 2초마다 갱신합니다. JS는 브라우저가 제공하는 추정 사용량, 엔진 할당은 문서 엔진이 확보한 메모리 공간입니다. 두 수치는 전체 탭 메모리가 아니며 합산하지 않습니다. MB는 1,000,000바이트입니다.</p></details>
  </div>;
}
