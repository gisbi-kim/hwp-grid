...
export default function Home(){
  ...
  const open=useCallback(async(file:File,restoreKey?:string)=>{
    ...
    }catch(error){if(id===loadId.current){const raw=String(error instanceof Error?error.message:error);if(import.meta.env.DEV)console.warn('HWP Grid:',raw);setNotice(/password|encrypt|암호|비밀번호/i.test(raw)?'암호가 걸린 문서는 현재 지원하지 않아. 한글에서 암호를 해제한 사본을 열어 줘.':/선택해|200 MB|빈 파일|페이지 수|크기를|브라우저에서/.test(raw)?raw:'문서를 열지 못했어. 손상되었거나 지원하지 않는 형식일 수 있어. 원본을 한글에서 확인해 줘.');}}
