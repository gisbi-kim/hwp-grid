...
import {cacheStats,clearDocumentCache,deleteCachedDocument} from '@/lib/document-cache';

const featureRequestUrl=`https://github.com/gisbi-kim/hwp-grid/issues/new?${new URLSearchParams({
  title:'[기능 요청] ',
  body:'## 원하는 기능\n어떤 기능이 필요한지 설명해 주세요.\n\n## 사용 상황\n어떤 상황에서 도움이 되는지 설명해 주세요.\n\n## 참고 사항 (선택)\n예시나 참고 자료가 있다면 적어 주세요. 개인정보나 민감한 문서는 첨부하지 말아 주세요.',
})}`;

const PageContent=memo(function PageContent({session,index,visible,priority,idSuffix}:{session:DocumentSession;index:number;visible:boolean;priority:boolean;idSuffix?:string}){
  ...
  return <div className="page-svg" aria-hidden="true" dangerouslySetInnerHTML={{__html:idSuffix?svg.replaceAll(`page-${session.key}-${index}-`,`thumb-${idSuffix}-${session.key}-${index}-`):svg}}/>;
});
const BookmarkThumbnail=memo(function BookmarkThumbnail({session,index}:{session:DocumentSession;index:number}){
  const ref=useRef<HTMLSpanElement>(null),id=useId().replace(/[^a-zA-Z0-9_-]/g,'');
  const [visible,setVisible]=useState(false);
  useEffect(()=>{const el=ref.current;if(!el)return;
    const observer=new IntersectionObserver(([entry])=>setVisible(entry.isIntersecting),{root:el.closest('.bookmark-list'),rootMargin:'120px'});
...
function CacheControls(){
  ...
  const refresh=()=>{setMessage('');setStats(null);void cacheStats().then(setStats).catch(()=>setMessage('이 브라우저에서는 캐시 저장소에 접근할 수 없습니다.'));};
  return <AlertDialog onOpenChange={open=>{if(open)refresh();}}><AlertDialogTrigger asChild><Button variant="ghost" size="sm">캐시 관리</Button></AlertDialogTrigger><AlertDialogContent className="delete-dialog"><AlertDialogHeader><AlertDialogTitle>이 컴퓨터의 문서 캐시</AlertDialogTitle><AlertDialogDescription>한 번 읽은 문서의 페이지 정보를 이 브라우저에 저장하여 다음에 더 빠르게 엽니다. 최대 10 GB이며, 공간이 부족하면 가장 오래 사용하지 않은 문서부터 자동 삭제합니다. 브라우저 저장 한도에 따라 실제 용량은 더 작을 수 있습니다.</AlertDialogDescription></AlertDialogHeader><p>{stats?stats.count+'개 문서 · '+(stats.bytes/1_000_000_000).toFixed(2)+' GB / 최대 10 GB':message?'':'사용량 확인 중…'}</p><p>캐시를 비워도 원본 파일, 마지막 문서 저장본, 책갈피는 유지됩니다. 암호 문서의 해제된 내용은 캐시하지 않습니다.</p>{message&&<p role="status">{message}</p>}<AlertDialogFooter><AlertDialogCancel>닫기</AlertDialogCancel><Button disabled={clearing||!stats?.count} onClick={async()=>{setClearing(true);try{await clearDocumentCache();setStats(await cacheStats());setMessage('캐시를 비웠습니다.');}catch{setMessage('캐시를 비우지 못했습니다.');}finally{setClearing(false);}}}>{clearing?'비우는 중…':'캐시 비우기'}</Button></AlertDialogFooter></AlertDialogContent></AlertDialog>;
}
...
function DocumentPane({slot,compare,onCompare}:{slot:'current'|'comparison';compare:boolean;onCompare:()=>void}){
  ...
  const [showMemos,setShowMemos]=useState(false),[memos,setMemos]=useState<DocumentMemo[]|null>(null),[memoError,setMemoError]=useState('');
  useEffect(()=>{setMemos(null);setMemoError('');},[session]);
  useEffect(()=>{if(!showMemos||!session)return;let active=true;void session.doc.memos().then(items=>{if(active)setMemos(items);}).catch(()=>{if(active)setMemoError('메모를 읽지 못했습니다. 원본을 한글에서 확인해 주세요.');});return()=>{active=false;};},[showMemos,session]);
  ...
  const open=useCallback(async(file:File,restoreKey?:string)=>{
    ...
    try{
      ...
      const previous=current.current;current.current=next;sourceFile.current=file;
  ...
  const clear=async()=>{saveView();++loadId.current;opening.current?.abort();setBusy('저장된 문서 삭제 중…');try{await clearFile(slot);try{localStorage.removeItem(viewKey);if(bookmarkStorageKey)writeBookmarks(bookmarkStorageKey,[]);}catch{}const previous=current.current;previous?.doc.free();if(previous?.cacheId)await deleteCachedDocument(previous.cacheId).catch(()=>{});current.current=null;setDocumentMs(null);setSession(null);setBookmarks([]);setBookmarkStorageKey('');setSaved(false);setZoom(1);setCols(2);setNotice('');if(previous)setTimeout(()=>previous.doc.free(),0);}catch{setNotice('저장된 문서를 삭제하지 못했습니다. 브라우저의 사이트 설정에서 삭제해 주세요.');}finally{setBusy('');}};
  ...
  return <section aria-label={slot==='current'?'첫 번째 문서':'두 번째 문서'} className="app document-pane" onDragEnter={e=>{e.preventDefault();if(e.dataTransfer.types.includes('Files')){dragDepth.current++;setDrop(true);}}} onDragOver={e=>e.preventDefault()} onDragLeave={e=>{e.preventDefault();if(--dragDepth.current<=0){dragDepth.current=0;setDrop(false);}}} onDrop={e=>{e.preventDefault();dragDepth.current=0;setDrop(false);if(e.dataTransfer.files[0])void open(e.dataTransfer.files[0]);}}>
    <header className="topbar"><div className="brand"><div className="brand-icon"><Grid2X2 size={22}/></div>{slot==='current'?<>HWP <span>Grid</span></>:<>비교 문서</>}</div><div className="filename" title={session?.name}>{session?.name||'한글 문서를 한눈에, 자유롭게.'}</div><Button onClick={()=>fileInput.current?.click()}><FolderOpen/>문서 열기</Button><Button variant="outline" disabled={!session||!!busy} aria-pressed={showMemos} onClick={()=>setShowMemos(v=>!v)}>{showMemos?'메모 숨기기':'메모 보기'}</Button><Button variant="outline" disabled={!session||!!busy} onClick={()=>{if(sourceFile.current)setEditFile(sourceFile.current);}}>편집 모드</Button><Button variant="secondary" aria-label={slot==='comparison'?'비교 닫기':compare?'비교 종료':'문서 비교'} onClick={onCompare}>{slot==='comparison'?<X/>:<Columns2/>}<span>{slot==='comparison'?'닫기':compare?'비교 종료':'문서 비교'}</span></Button>{slot==='current'&&<Button asChild variant="outline" size="sm" className="feature-request"><a href={featureRequestUrl} target="_blank" rel="noopener noreferrer" title="GitHub에서 기능 요청 작성 (새 탭)">기능 요청하기<span className="sr-only"> (GitHub, 새 탭)</span></a></Button>}<input ref={fileInput} type="file" accept=".hwp,.hwpx" aria-label="한글 문서 파일" hidden onChange={e=>{const file=e.target.files?.[0];e.target.value='';if(file)void open(file);}}/></header>
