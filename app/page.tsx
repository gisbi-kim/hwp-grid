...
function DocumentPane({slot,compare,onCompare}:{slot:'current'|'comparison';compare:boolean;onCompare:()=>void}){
  ...
  const go=(page:number)=>{pageInputDirty.current=false;const el=stage.current;if(!el||!session)return;const index=Math.max(0,Math.min(session.pages.length-1,Math.round(page)-1));el.scrollTop=layout.items[index].top-28;el.scrollLeft=Math.max(0,layout.items[index].left-28);setSelected(index);setPageInput(String(index+1));};
